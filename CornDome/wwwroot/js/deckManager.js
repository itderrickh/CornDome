...
async function displayDecks() {
    ...
    try {
        ...
        for (var i = 0; i < allDecks.length; i++) {
            ...
            var deckNameInput = `
                        <input class="name w-50" type="text" value="${allDecks[i].name}" />
            ...
            if (isGzipBase64(allDecks[i].content)) {
                // Trying to fix a bug that pops up for a select user
                //var content = encodeURIComponent(allDecks[i].content);
                //var linkContent = `<a href="/Deck?gzdeck=${content}" target="_blank">Link</a>`;
                //cell2.innerHTML = linkContent;

                const a = document.createElement("a");
                a.href = "/Deck?gzdeck=" + encodeURIComponent(allDecks[i].content);
                a.target = "_blank";
                a.textContent = "Link";

                cell2.replaceChildren(a);
            }
            else {
                // Trying to fix a bug that pops up for a select user
                //var content = allDecks[i].content;
                //var linkContent = `<a href="/Deck?deck=${content}" target="_blank">Link</a>`;
                //cell2.innerHTML = linkContent;

                const a = document.createElement("a");
                a.href = "/Deck?deck=" + allDecks[i].content;
                a.target = "_blank";
                a.textContent = "Link";
